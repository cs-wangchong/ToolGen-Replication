...
def handle(jsonl_path:str, max_len=128, tokenizer_name="Salesforce/codet5p-220m-py"):
    ...
    for line in tqdm(Path(jsonl_path).open("r").readlines(), ascii=True):
        ...
        if repo in train_repos:
            train_examples.append((docstring, signature, code, doc_len, code_len))
            augmented_train_examples.append((docstring, signature, augmented_code, lsp_count))
        elif repo in valid_repos:
            valid_examples.append((docstring, signature, code, doc_len, code_len))
            augmented_valid_examples.append((docstring, signature, augmented_code, lsp_count))
        elif repo in test_repos:
            test_examples.append((docstring, signature, code, doc_len, code_len))
            augmented_test_examples.append((docstring, signature, augmented_code, lsp_count))
            lsp_test_examples.append((repo, d["file_path"], context, line, column, docstring, signature, augmented_code))
    return (
            train_examples,
            valid_examples,
            test_examples,
...
if __name__ == '__main__':
    ...
    # train_tuples = [(docstring, signature, code, augmented_code, doc_len, code_len, lsp_count) for (docstring, signature, code, doc_len, code_len), (_, _, augmented_code, lsp_count) in zip(train_examples, augmented_train_examples)]
    # valid_tuples = [(docstring, signature, code, augmented_code, doc_len, code_len, lsp_count) for (docstring, signature, code, doc_len, code_len), (_, _, augmented_code, lsp_count) in zip(valid_examples, augmented_valid_examples)]
    # test_tuples = [(docstring, signature, code, augmented_code, doc_len, code_len, lsp_count) for (docstring, signature, code, doc_len, code_len), (_, _, augmented_code, lsp_count) in zip(test_examples, augmented_test_examples)]
    ...
    # train_code_len = 0
    # train_lsp_count = 0
    # for docstring, signature, code, augmented_code, doc_len, code_len, lsp_count in train_tuples:
    ...
    #     train_examples.append((docstring, signature, code))
    #     augmented_train_examples.append((docstring, signature, augmented_code))
    ...
    # valid_code_len = 0
    # valid_lsp_count = 0
    # for docstring, signature, code, augmented_code, doc_len, code_len, lsp_count in valid_tuples:
    #     if (docstring, signature) in visited_train_prompts:
    #         continue
    #     valid_doc_len += doc_len
    #     valid_code_len += code_len
    #     valid_lsp_count += lsp_count
    #     valid_examples.append((docstring, signature, code))
    #     augmented_valid_examples.append((docstring, signature, augmented_code))

    # test_examples, augmented_test_examples = [], []
    # test_doc_len = 0
    # test_code_len = 0
    # test_lsp_count = 0
    # for docstring, signature, code, augmented_code, doc_len, code_len, lsp_count in test_tuples:
    #     if (docstring, signature) in visited_train_prompts:
    #         continue
    #     test_doc_len += doc_len
    #     test_code_len += code_len
    #     test_lsp_count += lsp_count
    #     test_examples.append((docstring, signature, code))
    #     augmented_test_examples.append((docstring, signature, augmented_code))

    # lsp_test_examples = [(repo, path, context, line, column, docstring, signature, code) for repo, path, context, line, column, docstring, signature, code in lsp_test_examples if (docstring, signature) not in visited_train_prompts]
    ...
    new_lsp_test_examples = []
    for repo, path, docstring, signature, code in tqdm(lsp_test_examples):
        context, line, column = get_lsp_context(path, signature)
        if context is None:
            continue
        new_lsp_test_examples.append((repo, path, context, line, column, docstring, signature, code))
    lsp_test_examples = new_lsp_test_examples
    with Path("data/datasets/lsp-test.bin").open("wb") as f:
        pickle.dump(lsp_test_examples, f)
    with Path("data/datasets/lsp-test.json").open("w") as f:
        json.dump(list(lsp_test_examples), f, indent=4)

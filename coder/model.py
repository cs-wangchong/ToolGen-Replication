...
from transformers import AutoModelForCausalLM, LlamaForCausalLM, T5ForConditionalGeneration, GPT2LMHeadModel
...
def init_codellama(
    ...
    def extract_features(docstr, signature, code, max_len):
        ...
        features["labels"] = [(l if l != tokenizer.pad_token_id else -100) for l in features["labels"]]
        return features
...
def init_codet5(
    ...
    def extract_features(docstr, signature, code, max_len):
        input_text = build_prompt(docstr, signature)
        features = tokenizer(
            input_text,
            truncation=True,
            max_length=max_len,

...
def evaluate(
    ...
    if pred_result_file is None or not Path(pred_result_file).exists():
        ...
        if not call_lsp:
            ...
            for beg, end in tqdm(batch_ranges, ascii=True, desc="Evaluation"):
                ...
                _, _, _, _, _, docstrs, signatures, _ = zip(*batch)
                outputs = generator.generate_simple(docstrs, signatures, max_len=max_len, repetition_penalty=repetition_penalty)
                predictions.extend(outputs)
        else:
            repo2insts = defaultdict(list)
            for idx, (repo, file_path, context, line, column, docstr, signature, code) in enumerate(eval_set, 1):
                repo2insts[repo].append((idx, file_path, context, line, column, docstr, signature, code))
            for repo, insts in tqdm(repo2insts.items(), total=len(repo2insts), ascii=True, desc="Evaluation"):
                ...
                for beg, end in batch_ranges:
                    batch = insts[beg:end]
                    idxes, file_paths, contexts, lines, columns, docstrs, signatures, _ = zip(*batch)
                    # logging.info(f"====================================")
    ...
    return compute_metrics(generator.tokenizer, eval_set, predictions, lint_result_file, record_file)

def lint(eval_set, predictions):
    lint_results = []
    for (repo, file_path, context, line, column, docstr, signature, expt), pred in tqdm(zip(eval_set, predictions), desc="Lint", total=len(eval_set), ascii=True):
        errors = []
...
def compute_metrics(tokenizer, eval_set, predictions, lint_result_file=None, record_file=None):
    cleaned_preds = [clean_str(clean_lsp(code, strip=True)) for code in predictions]
    cleaned_expts = [clean_str(clean_lsp(expt, strip=False)) for _, _, _, _, _, _, _, expt in eval_set]
    infos = []
    for (repo, file_path, context, line, column, docstr, signature, expt), pred, cleaned_expt, cleaned_pred in zip(eval_set, predictions, cleaned_expts, cleaned_preds):
        info = f"======================\n[PROMPT]:\n{docstr}\n[Signature]:\n{signature}\n\n[EXPECTATION]:\n{expt}\n\n{cleaned_expt}\n\n[PREDICTION]:\n{pred}\n\n{cleaned_pred}\n\n"
        infos.append(info)
    ...
    edit_sim /= len(eval_set) 
    logging.info(f"[EDIT] edit similarity: {round(edit_sim, 4)}")

    total_count, hit_count = 0, 0
    hit = []
    idx = 0
    for (_, _, _, _, _, docstr, signature, expt), pred  in tqdm(zip(eval_set, predictions), desc="LSP Hit", total=len(eval_set), ascii=True):
        ...
        hit.append(0 if len(key_eles) == 0 else _hit_count / len(key_eles))
    ...
    if lint_result_file is None or not Path(lint_result_file).exists():
        lint_results = lint(eval_set, predictions)
    ...
    for idx, errors in tqdm(enumerate(lint_results), desc="Lint", total=len(eval_set), ascii=True):
        lines = []
        err_symbols = set()
    ...
    if record_file is not None:
        with Path(f"{record_file}").open("w") as f:
            f.write("\n\n".join(infos))
    syntax_pass_rate = syntax_pass / len(eval_set)
    semantic_pass_rate = semantic_pass / len(eval_set)
    logging.info(f"[LINT] total: {len(eval_set)}, syntax pass: {syntax_pass}, syntax pass rate: {syntax_pass_rate}")
    logging.info(f"[LINT] total: {len(eval_set)}, semantic pass: {semantic_pass}, semantic pass rate: {semantic_pass_rate}")
    
    os.unlink(f"{record_file}.basic")
    os.unlink(f"{record_file}.lsphit")
    return bleu, codebleu, edit_sim, hit_rate, syntax_pass_rate, semantic_pass_rate
...
from coder.generator_codellama import CodeLlamaGenerator
...
class LSPCoder:
    def __init__(self, generator:CodeLlamaGenerator):
        ...
        with torch.no_grad():
            for beg, end in tqdm(batch_ranges, ascii=True, desc="Evaluation"):
                ...
                for repo, file_path, desc, code in batch:
                    ...
                    prompt = reformat_prompt(desc)
                    if context is None:
                        output = self.generator.generate([prompt], beam_size=beam_size, max_len=max_len, repetition_penalty=repetition_penalty)[0]
                    else:
                        output = self.generate_with_lsp(prompt, file_path, context, line, column, max_len, beam_size, cand_num, lsp_threshold, token_threshold, token_k, temperature, repetition_penalty)
    ...
    def _init_decoding(self, source_ids, attention_mask, beam_size=2):
        init_id = torch.tensor([self.generator.tokenizer.bos_token_id], dtype=torch.long, device=self.device)
        with torch.no_grad():
            outputs: CausalLMOutputWithPast = self.generator.model(
        ...
        for i in range(beam_size):
            beam_decoding_ids.append(torch.cat([init_id, best_idx[i:i+1]], -1))
    ...
    def _beam_advance(
        ...
        with torch.no_grad():
            outputs: CausalLMOutputWithPast = self.generator.model(
                input_ids = decoding_inputs.unsqueeze(0)[:,-1:],
                # input_ids = decoding_inputs.unsqueeze(0)[:,:],
    ...
    def _lsp_expand(
        ...
        for cand in all_cands:
            trie.insert([self.generator.tokenizer.convert_tokens_to_ids([t])[0] for t in self.generator.tokenizer.tokenize(cand)])
            ...
            for beg in range(0, step_size, expand_batch_size):
                ...
                with torch.no_grad():
                    outputs = self.generator.model(
                        input_ids = lsp_decoder_input_ids[beg:end,-1:],
                        # input_ids = lsp_decoder_input_ids[beg:end,:],
    ...
    def generate_with_lsp(
        ...
        if token_k:
            token_k = min(token_k, len(self.generator.tokenizer))
        ...
        source_ids = source_ids[:,:-1] # remove </s>
        ...
        beam_decoding_ids, beam_past_key_values, beam_probs, beam_scores = self._init_decoding(source_ids, attention_mask, beam_size)

        final_generation_ids = []
        for step_idx in range(1, max_len):
            for beam_idx, decoding_ids in enumerate(beam_decoding_ids):
                # logging.info(f"beam-{beam_idx}")
                # logging.info(self.generator.tokenizer.decode(decoding_ids, skip_special_tokens=False).strip())
        ...
        logging.info("")
        output = final_generations[0][0].strip()
        return output
